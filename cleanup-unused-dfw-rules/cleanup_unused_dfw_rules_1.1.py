...
def create_policy_mp_mapping():
    ...
    log_and_print_message("Starting the process to create the policy-to-MP ID mapping file...", None)
    log_and_print_message(f"Querying NSX API at {url}", None)
    
    while url:
        try:
            response = requests.get(url, headers=headers, auth=(NSX_USERNAME, NSX_PASSWORD), verify=False)
            log_and_print_message(f"Received response from API, status code: {response.status_code}", None)
            if response.status_code != 200:
                log_and_print_message(f"Error: Received non-200 response: {response.status_code}", None)
                return  # Early exit if the API call fails
            
            data = response.json()
            log_and_print_message(f"Data received: {len(data.get('results', []))} items", None)
            ...
            if cursor:
                ...
                log_and_print_message(f"Paginated results found. Fetching more data with cursor: {cursor}", None)
            else:
                url = None
        except Exception as e:
            log_and_print_message(f"Exception occurred during API call: {str(e)}", None)
            return

    if not mapping:
        log_and_print_message("No data retrieved from the API. Skipping file creation.", None)
        return

    try:
        with open(MAPPING_FILE, 'w') as file:
            json.dump(mapping, file, indent=4)
        log_and_print_message(f"Policy-to-MP ID mapping saved to {MAPPING_FILE}", None)
    except Exception as e:
        log_and_print_message(f"Failed to write the mapping file: {str(e)}", None)


# Function to load the policy to MP ID mapping from the JSON file
def load_policy_mp_mapping():
    log_and_print_message(f"Attempting to load policy-to-MP ID mapping from {MAPPING_FILE}", None)
    if not os.path.exists(MAPPING_FILE):
        log_and_print_message(f"Mapping file not found at {MAPPING_FILE}. Attempting to create it.", None)
        create_policy_mp_mapping()

    if not os.path.exists(MAPPING_FILE):
        log_and_print_message(f"Mapping file still does not exist after attempt to create it.", None)
        sys.exit(1)  # Exit if the file cannot be found or created

    try:
        with open(MAPPING_FILE, 'r') as file:
            log_and_print_message(f"Successfully loaded mapping file from {MAPPING_FILE}", None)
            return json.load(file)
    except Exception as e:
        log_and_print_message(f"Error occurred while loading the mapping file: {str(e)}", None)
        sys.exit(1)
